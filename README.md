...
yatube │ ├── .venv/ │ └─ Виртуальное окружение Python │ ├── yatube ← Главный проект Django │ ├── init.py → делает папку Python-пакетом │ ├── asgi.py → запуск через ASGI (WebSocket, async) │ ├── settings.py → настройки проекта │ ├── urls.py → главные маршруты сайта │ └── wsgi.py → запуск через WSGI │ ├── posts/ ← Приложение "посты" │ ├── migrations/ → миграции БД │ ├── init.py → пакет Python │ ├── admin.py → регистрация моделей в админке │ ├── apps.py → настройка приложения │ ├── forms.py → Django-формы │ ├── models.py → модели БД │ ├── serializers.py → сериализация данных (API/DRF) │ ├── tests.py → тесты │ ├── urls.py → маршруты приложения │ └── views.py → логика обработки запросов │ ├── api/ ← API приложения │ └─ эндпоинты для работы с данными │ ├── users/ ← приложение пользователей │ └─ регистрация, авторизация, профиль │ ├── templates/ ← HTML-шаблоны │ ├── about/ → страницы "О нас" │ ├── includes/ → общие части шаблонов │ ├── posts/ → шаблоны постов │ ├── users/ → шаблоны пользователей │ └── base.html → основной шаблон │ ├── static/ ← CSS, JS, изображения │ ├── db.sqlite3 → база данных SQLite │ ├── manage.py → главная команда Django │ ├── requirements.txt → зависимости проекта │ ├── .gitignore → что не отправлять в Git │ ├── README.md → описание проекта │ └── run_windows.sh → скрипт запуска
=======
>>>>>>> 53a0fc7c6c54a49d9b8f1d684997b3bd926b0023
